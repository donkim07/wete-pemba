...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">{{ __('Edit News Article') }}</h1>
        <a href="{{ route('admin.government.news.index') }}" class="d-none d-sm-inline-block btn btn-sm btn-secondary shadow-sm">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">{{ __('News Article Details') }}</h6>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <form action="{{ route('admin.government.news.update', $news->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('short_description') is-invalid @enderror" id="short_description" name="short_description" rows="2">{{ old('short_description', $news->excerpt) }}</textarea>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="form-group">
                            ...
                            <div id="image-preview-container">
                                @if($news->featured_image)
                                    <img src="{{ asset('images/' . $news->featured_image) }}" class="image-preview mt-2" alt="{{ $news->title }}">
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <div class="form-check">
                                <input class="form-check-input @error('is_featured') is-invalid @enderror" type="checkbox" id="is_featured" name="is_featured" value="1" {{ old('is_featured', $news->is_featured) ? 'checked' : '' }}>
                        ...
                        <div class="form-group">
                            <div class="form-check">
                                <input class="form-check-input @error('is_ticker') is-invalid @enderror" type="checkbox" id="is_ticker" name="is_ticker" value="1" {{ old('is_ticker', $news->is_ticker) ? 'checked' : '' }}>
                        ...
                        <div class="form-group">
                            <div class="form-check">
                                <input class="form-check-input @error('is_critical') is-invalid @enderror" type="checkbox" id="is_critical" name="is_critical" value="1" {{ old('is_critical', $news->is_critical) ? 'checked' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-select tag-selector @error('tags') is-invalid @enderror" id="tags" name="tags[]" multiple>
                                @foreach($tags as $tag)
                                    <option value="{{ $tag->id }}" {{ (is_array(old('tags', $news->tags->pluck('id')->toArray())) && in_array($tag->id, old('tags', $news->tags->pluck('id')->toArray()))) ? 'selected' : '' }}>
                                        {{ $tag->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="d-flex justify-content-end mt-4">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save me-1"></i> {{ __('Update News Article') }}
                    </button>
                </div>
            </form>
        </div>
