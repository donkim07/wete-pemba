        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.government.testimonials.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select @error('service_id') is-invalid @enderror" id="service_id" name="service_id">
                                                ...
                                                @foreach($services as $id => $title)
                                                    <option value="{{ $id }}" {{ old('service_id', $testimonial->service_id) == $id ? 'selected' : '' }}>{{ $title }}</option>
                                                @endforeach
                                            </select>
                                            @error('service_id')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select @error('rating') is-invalid @enderror" id="rating" name="rating">
                                                ...
                                                @for($i = 1; $i <= 5; $i++)
                                                    <option value="{{ $i }}" {{ old('rating', $testimonial->rating) == $i ? 'selected' : '' }}>{{ $i }} {{ __('Star(s)') }}</option>
                                                @endfor
                                            </select>
                                            @error('rating')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                    ...
                                    <div class="card-body">
                                        @if($testimonial->avatar)
                                            <div class="text-center mb-3">
                                                <img src="{{ asset('storage/' . $testimonial->avatar) }}" alt="{{ $testimonial->name }}" class="img-thumbnail" style="max-height: 200px;">
                                                <div class="mt-2">
                                                    <div class="form-check">
                                        ...
                                        <div class="mb-3">
                                            <label for="avatar" class="form-label">{{ __('Upload New Avatar') }}</label>
