    ...
    <div class="row">
        ...
        <div class="col-lg-3 mb-4">
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body p-0">
                    <div class="list-group list-group-flush">
                        <a href="{{ route('government.news.index') }}" class="category-link {{ !request('category') ? 'active' : '' }}">
                            <i class="fas fa-newspaper"></i> All News
                            <span class="badge rounded-pill">{{ $totalNews ?? count($news ?? []) }}</span>
                        </a>
                        
                        @foreach($categories as $category)
                        <a href="{{ route('government.news.index', ['category' => $category->id]) }}" 
                ...
                <div class="card-body">
                    <div class="d-flex flex-wrap">
                        @if(isset($tags) && count($tags) > 0)
                            @foreach($tags as $tag)
                            <a href="{{ route('government.news.index', ['tag' => $tag->id]) }}" class="news-tag mb-2 me-2">
                                #{{ $tag->name }}
                            </a>
                            @endforeach
                        @else
            ...
            <div class="card border-0 shadow-sm mt-4">
                ...
                <div class="card-body">
                    <form action="{{ route('government.news.index') }}" method="GET" id="news-filter-form">
                        @if(request('category'))
        ...
        <div class="col-lg-9">
            <!-- Featured News -->
            @if(isset($featuredNews) && $featuredNews && !request('search') && !request('tag') && !request('year') && (!request('category') || request('page')))
            ...
            @if(request('search') || request('year') || request('tag'))
                <div class="alert alert-info mb-4">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('government.news.index', request('category') ? ['category' => request('category')] : []) }}" class="btn btn-sm btn-outline-primary">
                                ...
                                @if($newsItem->tags && $newsItem->tags->count() > 0)
                                    <div class="mb-3">
                                        @foreach($newsItem->tags as $tag)
                                            <a href="{{ route('government.news.index', ['tag' => $tag->id]) }}" class="news-tag">
                                                #{{ $tag->name }}
                                            </a>
                                        @endforeach
                                    </div>
